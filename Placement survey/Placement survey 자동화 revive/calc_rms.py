# -*- coding: utf-8 -*-
"""
JK Placement Survey - RMS 계산 + 결과 시트 생성
Usage: python calc_rms.py --input "JK 전체/25Q4_JK_결과.xlsx"
"""
import sys
...
# Constants
# ============================================================
CHANNELS = ['잡코리아', '사람인', '원티드', '리멤버', '기타 온라인', '오프라인', '미분류']
CHANNELS_ONLINE = ['잡코리아', '사람인', '원티드', '리멤버', '기타 온라인']
CHANNELS_4 = ['잡코리아', '사람인', '원티드', '리멤버']
REASON_CATS = ['채용 공고', '유용한 정보 제공', '이용 편의성', '부가 서비스',
               '공공/제도/신뢰', '브랜드/평판', '기타']

CUBICLE_ORDER = [
    '스킬기반산업 (스)2030 서울경기강원', '스킬기반산업 (스)4050 전국',
    '스킬기반산업 (스)2030 영남충청호남', '기타산업 기타 전국',
    '현장중심산업 (현)20~40 서울경기강원', '현장중심산업 (현)50 전국',
    '현장중심산업 (현)20~40 영남충청호남', '현장중심산업 (현)60+ 전국',
    '전문직군산업 (전)20~50 전국', '자격중심산업 (자)20~50 전국',
    '공공형산업 (공)20~40 서울경기강원영남', '공공형산업 (공)50 전국',
    '공공형산업 (공)20~40 충청호남',
    '프로젝트형산업 (프)2030 전국', '프로젝트형산업 (프)40 전국', '프로젝트형산업 (프)50 전국',
    '대량채용산업 (대)20~40 영남충청호남', '대량채용산업 (대)20~40 서울경기강원',
    '대량채용산업 (대)50 전국', '상시채용산업 (상)2030 전국',
    '전문직군산업 (전)60+ 전국', '대량채용산업 (대)60+ 전국',
    '스킬기반산업 (스)60+ 전국', '공공형산업 (공)60+ 전국',
    '상시채용산업 (상)4050 전국', '프로젝트형산업 (프)60+ 전국', '자격중심산업 (자)60+ 전국',
]

SCOPES = [
    ('전체 (Online+Offline)', CHANNELS),
    ('ONLINE only', CHANNELS_ONLINE),
    ('JK SRI WTD RMB', CHANNELS_4),
]

SEG_DIMS = [
    ('seg_ind', '산업'), ('seg_job', '직무'),
    ('seg_income', '소득수준'), ('seg_region', '지역'),
]
...
def load_data(filepath):
    ...
    rename = {
        ...
        'Seg 산업': 'seg_ind', 'Seg 직무': 'seg_job',
        'Seg 소득수준': 'seg_income', 'Seg 지역': 'seg_region',
    }
...
def calc_rms(df, ch_col, quarters):
    ...
    # Adjusted: unadjusted * average(weight across prior quarters)
    prior = quarters[:-1]
    adj = {ch: {q: {} for q in quarters} for ch in CHANNELS}
    for ch in CHANNELS:
        for q in quarters:
            for cub in CUBICLE_ORDER:
                pw = [weight[pq].get(cub, 0) for pq in prior if pq in weight]
                avg_w = np.mean(pw) if pw else 0
                adj[ch][q][cub] = unadj[ch][q].get(cub, 0) * avg_w
...
def write_rms_sheet(ws, name, rms, quarters):
    ...
    for adj_label, data, start_row in [('Adjusted RMS', ta, 2), ('Unadjusted RMS', tu, None)]:
        ...
        # JK/(JK+SRI)
        sc(ws, r, 1, 'JK/(JK+SRI)', bold=True)
        for i, q in enumerate(quarters):
            jk = data['잡코리아'].get(q, 0)
            sri = data['사람인'].get(q, 0)
            sc(ws, r, i + 3, jk / (jk + sri) if (jk + sri) else 0, fmt=PCT)
        r += 1
...
def write_reason_sheet(ws, title, df, ch_col, rcols, quarters, exclude_quarters=None):
    ...
    blocks = [(None, '전체'), ('잡코리아', '잡코리아'), ('사람인', '사람인'),
              ('리멤버', '리멤버'), ('원티드', '원티드')]
    r = 4
...
def write_seg_reason_sheet(ws, title, df, ch_col, rcols, quarters, exclude_quarters=None):
    ...
    for seg_col, seg_name in SEG_DIMS:
        ...
        for seg_val in seg_vals:
            ...
            channels_to_check = CHANNELS_4  # 4사만
            first_ch = True
...
def write_seg_insight(ws, df, ch_col, quarters):
    ...
    for seg_col, seg_name in insight_dims:
        ...
        for seg in seg_vals:
            ...
            p_jk_sri = p_rms_vals['잡코리아'] - p_rms_vals['사람인']
...
def main():
    ...
    for name, col, _ in types:
        ...
        jk = rms_results[name]['t_adj']['잡코리아'].get(quarters[-1], 0)
        print(f"  {quarters[-1]} JK Adj={jk:.4f}")
    ...
    # 채널 선택 이유
    # 지원: 22Q2 제외 (이유 데이터 없음)
    ws = wb.create_sheet('지원 채널 선택 이유')
    write_reason_sheet(ws, '지원 채널 선택 이유', df, 'ch_apply',
                       ['ra1', 'ra2', 'ra3'], quarters, exclude_quarters=['22Q2'])

    # 재지원: 25Q2 이전 제외 (25Q3부터 수집)
    reuse_exclude = [q for q in quarters if q <= '25Q2']
